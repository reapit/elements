...
- **feat:** Add codemod for migrating `AtAGlance.Card` to `AtAGlance.ArticleCard`. The codemod is provided as a binary from Elements, so consumers can run `yarn codemod list` to see details about available codemods.
- **refactor!:** Add `HTMLDialog` utility component and refactor `Drawer`, `Dialog` and `Combobox.Popup` components to use it internally. Consolidated dialog-related utilities into `src/utils/dialog` (previously scattered across component directories). Deprecated old utility function names in favour of more descriptive names: `useDialogController` → `useDialogOpenController`, `useDialogObserver` → `useDialogOpenState`, `openComboboxPopup` → `openDialog`, `closeComboboxPopup` → `closeDialog`. Old names remain available as deprecated re-exports for backwards compatibility.
