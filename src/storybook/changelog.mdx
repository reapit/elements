...
We are also ope n to feature requests - in the same way please request [here](https://github.com/reapit/elements/issues/new?assignees=&labels=needs-triage%2C+feature&projects=&template=feature_request.md&title=) - if it makes sense with our roadmap and the wider design system, we will accept for development.
...
- Below is the detailed changelog for individual components that will be replaced in v5.
    **Avatar**
        - Breaking change - The old `Avatar` renamed to `DeprecatedAvatar`
        - `type` prop has been renamed to `shape`.
        - `src` prop no longer supported, use `children` or use `AvatarRectangle` if needed.
        - Detailed props and usage are available in Storybook under the `Avatar` component.

    **Badge**
        - **Breaking change** - The Badge component has be refactored with updated props.
        - The `intent` prop has been deprecated. Use `variant` as a replacement with new values.
        - Details on new and modified prop usage are available in Storybook under the Badge component.

    **BreadCrumb**
        - Breaking change - The old `BreadCrumb` renamed to `DeprecatedBreadCrumb`
        - The new `Breadcrumb` component is still in progress

    **Button**
        - **Breaking change** - The `Button` component has been refactored with updated props.
        - The `intent` prop has been deprecated. Use `variant` as a replacement with new values.
        - Details on new and modified prop usage are available in Storybook under the Button component.

    **Chip/ChipGroup**
        - Updates primarily focus on UI presentation.
        - The Chip component now supports additional props: variant, willTruncateLabel, and isDisabled.
        - The ChipGroup component now includes support for wrapping or scrolling.
    ...
    **Pagination**
        - **Breaking change** - `callback` prop has been renamed to `onPageChange`, without changing the behaviour.
        - `currentPage` prop still remains with the same functionality.
        - `numberPages` prop has been deprecated. Use `totalPages` instead.
        - `hasStartButton` and `hasEndButton` props are no longer supported.
        - Details on new and modified prop usage are available in Storybook under the `Pagination` component.

    **StatusIndicator**
        - **Breaking change** - The `variant` prop has been deprecated. Use `intent` instead.
        - `shape` prop is no longer supported.
        - Details on new and modified prop usage are available in Storybook under the `StatusIndicator` component.

    **Table**
        - **Breaking Change** - This component is currently in progress and undergoing staged releases; it is not yet readily available for use.
        - The v4 Table is still accessible as DeprecatedTable.
        - The new Table component will introduce more customizable props at the atomic level.
        - Details on the new Table atoms can be found in Storybook under their respective elements.

    **Tag**
        - **Breaking change** - The old `Tag` renamed to `DeprecatedTag`
        - `intent` prop no longer supported
        - Detailed props and usage are available in Storybook under the `Tag` component

    **Tooltip**
        - **Breaking change** - Moving from v4 -> v5, v4 props are replaced with new v5 props.
        - Details on new and modified prop usage are available in Storybook under the Tooltip component.
