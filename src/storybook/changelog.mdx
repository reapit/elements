...
- **feat:** Added new `PageHeader.LeadingElement` after design updates to the page header. See [PageHeader.LeadingElement](?path=/docs/components-pageheader-leadingelement--docs) for details. Avatar's are no longer supported in the new page header.
...
- **feat:** Add new `CompactSelectNative` component. See [CompactSelectNative](?path=/docs/components-compactselectnative--docs) for details.
...
- **feat:** Add new `PageHeader` component. See [PageHeader](?path=/docs/components-pageheader--docs) for details.
...
- **feat:** Add new `Button` component. See [Button](?path=/docs/components-button--docs) for details.
- **chore!:** Refactor `Tag` and `TagGroup` to match the structure of `Chip` and `ChipGroup`. `Tag` no longer supports an `as` prop. See [Tag](?path=/docs/components-tag--docs) and [TagGroup](?path=/docs/components-tag-group--docs) for details.
- **chore!:** Refactor `Badge` component, with the following breaking changes: `variant` is now `colour` and `isReversed` is now `variant`. See [Badge](?path=/docs/components-badge--docs) for details.
- **feat!:** `Features` component now provides common feature items "out-of-the-box". See [Features](?path=/docs/components-features--docs) for details. The low-level `Features.Item` component also received a breaking change, with `aria-label` being replaced with `label`.
