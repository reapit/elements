...
test('`<details>` element is opened when the `SideBar` is expanded and a descendant represents the current page', () => {
  ...
  rerender(
    // NOTE: the `open` attribute is still absent when we rerender with the SideBar expanded, but we expect the <details>
    // element to be opened by our useSideBarMenuGroupController because one of its descendants represents the current page
...
test('`<details>` element is closed when the `SideBar` is collapsed', () => {
  const { rerender } = render(
...
test('`<details>` element is opened when a descendant comes to represent the current page', async () => {
  const { rerender } = render(
    // NOTE: the `open` attribute is absent when we first render
    <Details sideBarState="expanded">
...
test('`<details>` element is closed when a descendant no longer represents the current page', async () => {
  const { rerender } = render(
  ...
  rerender(
    ...
    // closed by our useSideBarMenuGroupController because one of its descendants _no longer_ represents the current page
    <Details open sideBarState="expanded">
