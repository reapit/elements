...
test('a consumer-supplied `onClick` can prevent the side bar state from being toggled', async () => {
  render(<CollapseButtonStoriesWithSideBarInitiallyCollapsed.Example onClick={(event) => event.preventDefault()} />)

  // The button has an accessible name of "Expand" when we render, because the SideBar is initially collapsed.
  fireEvent.click(screen.getByRole('button', { name: 'Expand' }))

  // We expect the button to still have an accessible name of "Expand", because the default action (toggling the sidebar)
  // was prevented by our onClick handler.
  expect(screen.getByRole('button', { name: 'Expand' })).toBeVisible()
})
