...
test('displays error text, when provided', () => {
  render(<CurrencyControl label="Amount" currency="GBP" helpText="Help text" errorText="Error text" />)
...
test('forwards additional attributes to the input', () => {
  render(<CurrencyControl data-testid="test-id" label="Amount" currency="GBP" placeholder="0.00" />)
...
test('respects an explicit showValidity={false} override even when error text is present', () => {
  render(<CurrencyControl label="Amount" currency="GBP" errorText="Error text" showValidity={false} />)
...
test('displays a symbol-free formatted overlay and renders the currency symbol as an affix', () => {
  const { container } = render(<CurrencyControl label="Amount" currency="GBP" locale="en-GB" value="1234.5" />)
