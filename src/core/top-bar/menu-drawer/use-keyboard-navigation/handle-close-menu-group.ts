...
export function handleCloseMenuGroup(event: KeyboardEvent<HTMLElement>) {
  ...
  if (event.target instanceof HTMLElement) {
    ...
    // We only prevent default if the event target is a descendant of a <details> element
    // and is NOT the <summary> element.
    if (detailsElement?.open && event.target !== summaryElement) {
    ...
    // We only close the <details> element if it does NOT have descendants that represent the current page.
