...
test('child element has data-template attribute set to specified template', () => {
  ...
  expect(screen.getByRole('article').firstElementChild).toHaveAttribute('data-template', 'two-columns-asymmetrical-end')
})
...
test.skip('applies specified background colour', () => {
  const { container } = render(<MainContainer backgroundColour="--colour-fill-neutral-lightest" size="narrow" />)
  expect(container.firstElementChild).toHaveStyle({ backgroundColor: 'var(--colour-fill-neutral-lightest)' })
})
