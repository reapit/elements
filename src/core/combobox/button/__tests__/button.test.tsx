...
test('displays leading icon when supplied', () => {
  render(<ComboboxButton aria-controls="popup" aria-expanded={false} id="my-button" leadingIcon="leading icon" />)
  expect(screen.getByText('leading icon')).toBeVisible()
})

test('displays action when supplied', () => {
  render(<ComboboxButton action="action" aria-controls="popup" aria-expanded={false} id="my-button" />)
...
test('forwards `className` to the root container element', () => {
  const { container } = render(
    <ComboboxButton aria-controls="popup" aria-expanded={false} className="my-class" id="my-button" />,
  )
...
test('forwards `style` to the root container element', () => {
  const { container } = render(
    <ComboboxButton aria-controls="popup" aria-expanded={false} style={{ color: 'red' }} id="my-button" />,
  )
  expect(container.firstElementChild).toHaveStyle('color: red')
})

test('forwards additional props to the button element', () => {
  render(<ComboboxButton aria-controls="popup" aria-expanded={false} data-testid="test-id" id="my-button" />)
