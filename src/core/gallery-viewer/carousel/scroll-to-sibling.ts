 ...
 */
export function scrollToSibling(trackEl: HTMLElement, activeItemId: string, direction: 'previous' | 'next'): boolean {
  const activeEl = trackEl.querySelector(`#${CSS.escape(activeItemId)}`)
  if (!activeEl) return false

  const sibling = direction === 'next' ? activeEl.nextElementSibling : activeEl.previousElementSibling
