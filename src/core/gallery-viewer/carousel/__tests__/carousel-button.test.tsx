...
test('sets data-direction on the buttons', () => {
  render(<TestComponent />)
  expect(screen.getByRole('button', { name: 'Previous' })).toHaveAttribute('data-direction', 'previous')
...
test('calls scrollIntoView on the next sibling when the next button is clicked', () => {
  render(<TestComponent />)
  configureTrack(screen.getByTestId('track'), { scrollLeft: 100, clientWidth: 100, scrollWidth: 300 })
...
test('does not call scrollIntoView when the consumer calls event.preventDefault()', () => {
  render(
    <GalleryViewerCarousel aria-label="Prevented carousel" defaultValue="item-1">
      <GalleryViewerCarouselButton aria-label="Next (no scroll)" direction="next" onClick={(e) => e.preventDefault()} />
  ...
  configureTrack(screen.getByTestId('track-prevented'), { scrollLeft: 100, clientWidth: 100, scrollWidth: 300 })
...
test('calls the consumer onClick before scrolling', () => {
  ...
  configureTrack(screen.getByTestId('track-order'), { scrollLeft: 100, clientWidth: 100, scrollWidth: 300 })
...
test('is disabled when the track is at the leftmost position (previous button)', async () => {
  ...
  configureTrack(screen.getByTestId('track'), { scrollLeft: 0, clientWidth: 100, scrollWidth: 300 })
...
test('is disabled when the track is at the rightmost position (next button)', async () => {
  ...
  configureTrack(screen.getByTestId('track'), { scrollLeft: 200, clientWidth: 100, scrollWidth: 300 })
...
test('is disabled when the carousel is in read-only controlled mode', () => {
  ...
  configureTrack(screen.getByTestId('track-readonly'), { scrollLeft: 100, clientWidth: 100, scrollWidth: 300 })
  expect(screen.getByRole('button', { name: 'Previous' })).toHaveAttribute('aria-disabled', 'true')
  expect(screen.getByRole('button', { name: 'Next' })).toHaveAttribute('aria-disabled', 'true')
})
...
test('a consumer-provided disabled prop overrides auto-disable', () => {
  ...
  configureTrack(screen.getByTestId('track-disabled'), { scrollLeft: 100, clientWidth: 100, scrollWidth: 300 })
  expect(screen.getByRole('button', { name: 'Next' })).toHaveAttribute('aria-disabled', 'true')
})
