...
test('renders the filename and thumbnail', () => {
  const { container } = render(<FileUploaderMediaCard fileName="Photo.jpg" src={src} status="uploaded" />)
...
test('renders the formatted file size and status text', () => {
  render(
    <FileUploaderMediaCard fileName="Photo.jpg" fileSize={2 * 1000 * 1000} locale="en-GB" src={src} status="queued" />,
  )
...
test('renders the error message when status is error', () => {
  render(<FileUploaderMediaCard errorMessage="File too large" fileName="Photo.jpg" src={src} status="error" />)
...
test('renders an accessibly-named remove button that calls onRemove when clicked', () => {
  ...
  render(<FileUploaderMediaCard fileName="Photo.jpg" src={src} status="uploaded" onRemove={onRemove} />)
...
test('renders an indeterminate spinner while uploading with no known progress', () => {
  const { container } = render(<FileUploaderMediaCard fileName="Photo.jpg" src={src} status="uploading" />)
...
test.each([NaN, Infinity])(
  ...
  (progress) => {
    const { container } = render(
      <FileUploaderMediaCard fileName="Photo.jpg" progress={progress} src={src} status="uploading" />,
    )
    expect(container.querySelectorAll('circle')).toHaveLength(2)
    expect(container.querySelector('path')).toBeNull()
  },
...
test('renders an indeterminate spinner while processing', () => {
  const { container } = render(<FileUploaderMediaCard fileName="Photo.jpg" src={src} status="processing" />)
  expect(container.querySelectorAll('circle')).toHaveLength(2)
})

test('renders no progress indicator while queued, uploaded, or errored', () => {
  for (const status of ['queued', 'uploaded', 'error'] as const) {
    const { container, unmount } = render(<FileUploaderMediaCard fileName="Photo.jpg" src={src} status={status} />)
...
test('renders the duration badge only when provided', () => {
  render(<FileUploaderMediaCard duration="15:39" fileName="Video.mp4" src={src} status="uploaded" />)
