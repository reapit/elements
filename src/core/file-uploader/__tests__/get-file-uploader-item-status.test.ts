...
test('returns "Uploading: n%" when progress is a number', () => {
  expect(getFileUploaderItemStatus({ status: "uploading", progress: 45 }).statusText).toBe(
    "Uploading: 45%",
  );
});

test("rounds a fractional progress percentage", () => {
  expect(getFileUploaderItemStatus({ status: "uploading", progress: 45.6 }).statusText).toBe(
    "Uploading: 46%",
  );
});
...
test("a present errorMessage is treated as an error even on a non-error status, for a failing validationError", () => {
  const result = getFileUploaderItemStatus({
    status: "queued",
...
test("clamps an out-of-range progress percentage", () => {
  expect(getFileUploaderItemStatus({ status: "uploading", progress: 120 }).statusText).toBe(
    "Uploading: 100%",
  );
});

test('returns "Uploading" with no percentage when progress is NaN or Infinity', () => {
  expect(getFileUploaderItemStatus({ status: "uploading", progress: Number.NaN }).statusText).toBe(
    "Uploading",
  );
  expect(
    getFileUploaderItemStatus({ status: "uploading", progress: Number.POSITIVE_INFINITY })
      .statusText,
  ).toBe("Uploading");
});
