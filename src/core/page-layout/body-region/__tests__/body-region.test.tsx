...
test('has data-overflow="auto" attribute when scroll="body"', () => {
  render(<PageLayoutBodyRegion />, {
    wrapper: ({ children }) => (
      <PageLayoutContext.Provider value={{ rootId: 'test-id', scroll: 'body' }}>{children}</PageLayoutContext.Provider>
    ),
  })
...
test('does NOT have data-overflow attribute when scroll="self"', () => {
  render(<PageLayoutBodyRegion />, {
    wrapper: ({ children }) => (
      <PageLayoutContext.Provider value={{ rootId: 'test-id', scroll: 'self' }}>{children}</PageLayoutContext.Provider>
    ),
  })
