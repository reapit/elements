...
test('commits a valid integer paste into an empty input', () => {
  const result = resolvePaste({ currentValue: '', selection: empty, sanitised: '1234', allowNegative: true })
  expect(result).toEqual({ type: 'commit', value: '1234', cursor: 4 })
})

test('commits a valid decimal paste into an empty input', () => {
  const result = resolvePaste({ currentValue: '', selection: empty, sanitised: '12.34', allowNegative: true })
  expect(result).toEqual({ type: 'commit', value: '12.34', cursor: 5 })
})

test('commits a negative paste when negatives are allowed', () => {
  const result = resolvePaste({ currentValue: '', selection: empty, sanitised: '-5', allowNegative: true })
  expect(result).toEqual({ type: 'commit', value: '-5', cursor: 2 })
})

test('rejects a negative paste when negatives are not allowed', () => {
  const result = resolvePaste({ currentValue: '', selection: empty, sanitised: '-5', allowNegative: false })
  expect(result).toEqual({ type: 'reject' })
})

test('rejects when sanitised text is empty', () => {
  const result = resolvePaste({ currentValue: '', selection: empty, sanitised: '', allowNegative: true })
  expect(result).toEqual({ type: 'reject' })
})

test('rejects when sanitised text contains an embedded minus', () => {
  const result = resolvePaste({ currentValue: '', selection: empty, sanitised: '1-2', allowNegative: true })
  expect(result).toEqual({ type: 'reject' })
})
