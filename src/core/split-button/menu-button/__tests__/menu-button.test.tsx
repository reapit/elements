...
test("is ARIA disabled when `SplitButtonContext` is no `busy` but `aria-disabled` is true", () => {
  render(<SplitButtonMenuButton aria-disabled={true} aria-label="Expand" aria-expanded={true} />, {
    wrapper: Wrapper,
  });
...
test("applies custom `className`", () => {
  render(<SplitButtonMenuButton aria-label="Menu" className="custom-class" />, {
    wrapper: Wrapper,
  });
