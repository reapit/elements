...
export function useSwipeToDismiss({
  ...
  const onPointerMove = useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      ...
      if (delta > 0) {
        ...
        swipeOffsetRef.current = delta
        contentElRef.current?.style.setProperty('--swipe-offset', `${String(dampened * swipeSign)}px`)
      }
    },
  ...
  const onPointerUp = useCallback(() => {
    ...
    if (swipeOffsetRef.current > 0 && (absDelta >= SWIPE_DISMISS_THRESHOLD || velocity > SWIPE_VELOCITY_THRESHOLD)) {
