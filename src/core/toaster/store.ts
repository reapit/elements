...
export const toastStore = {
  ...
  add: (entry: Omit<ToastEntry, 'id' | 'state' | 'startedAt'>): string => {
    ...
    const duration = isTimedDuration(entry.duration) ? Math.max(entry.duration, MIN_DURATION) : entry.duration
  ...
  settle: (id: string): void => {
    ...
    toasts = toasts.map((t) => (t.id === id ? { ...t, state: 'visible' as ToastState, startedAt } : t))
