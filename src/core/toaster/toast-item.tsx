...
export function ToastItem({
  ...
  const [elapsed] = useState(() =>
    isTimedDuration(duration) && startedAt !== null ? Math.min(Date.now() - startedAt, duration) : 0,
  )
  ...
  return (
    ...
    >
      ...
      >
        <Toast variant={variant} duration={duration} elapsed={elapsed} isPaused={isPaused} icon={icon}>
          {message}
        </Toast>
      </div>
    </li>
