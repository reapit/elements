...
test('removes the highlight on unmount', () => {
  ...
  const { unmount } = render(<TestComponent name="test-highlight" query="ipsum" text="Lorem ipsum dolor sit amet." />)
  expect(highlights.has('test-highlight')).toBe(true)
...
test('re-applies the highlight when the query changes', () => {
  ...
  const { rerender } = render(<TestComponent name="test-highlight" query="ipsum" text="Lorem ipsum dolor sit amet." />)
  const first = highlights.get('test-highlight')
  expect(first?.ranges).toHaveLength(1)

  rerender(<TestComponent name="test-highlight" query="dolor" text="Lorem ipsum dolor sit amet." />)
