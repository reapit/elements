...
test('sets aria-label to "Remove {label}"', () => {
  render(<ComboboxSelectionChip value="banana">Banana</ComboboxSelectionChip>, { wrapper: Wrapper })
...
test('calls setListboxOptionSelectedState with correct arguments when clicked', () => {
  ...
  expect(setListboxOptionSelectedState).toHaveBeenCalledWith('fruit-listbox', 'orange', expect.any(Function))
})
...
test('uses the value prop as the chip value', () => {
  render(<ComboboxSelectionChip value="pineapple-123">Pineapple</ComboboxSelectionChip>, { wrapper: Wrapper })
  const chip = screen.getByRole('button')
...
test('handles click on chip with special characters in label', () => {
  render(<ComboboxSelectionChip value="test-value">Test & Value</ComboboxSelectionChip>, { wrapper: Wrapper })

  fireEvent.click(screen.getByRole('button'))

  expect(setListboxOptionSelectedState).toHaveBeenCalledWith('test-listbox', 'test-value', expect.any(Function))
})
...
function Wrapper({ children, listboxId = 'test-listbox' }: WrapperProps) {
  return (
    <ChipGroup variant="selection">
      <ComboboxSelectionChipsContext.Provider value={{ listboxId }}>{children}</ComboboxSelectionChipsContext.Provider>
    </ChipGroup>
  )
}
