...
test('displays action when provided', () => {
  render(
    <ComboboxCard action={<button aria-label="Clear">×</button>} aria-controls="listbox" aria-expanded={false}>
      Content
    </ComboboxCard>,
  )
...
test('calls onClick handler when aria-disabled="false"', () => {
  const onClick = vi.fn()
  render(
    <ComboboxCard aria-controls="listbox" aria-disabled={false} aria-expanded={false} onClick={onClick}>
      Content
    </ComboboxCard>,
  )
