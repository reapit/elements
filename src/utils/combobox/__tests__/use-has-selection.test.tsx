...
test('returns true when listbox state has selections', async () => {
  let observerCallback: ((visibleOptions: HTMLButtonElement[], listboxState: readonly string[]) => void) | null = null
...
test('passes listboxId to useListboxSelectionObserver', () => {
  ...
  expect(mockUseListboxSelectionObserver).toHaveBeenCalledWith('my-custom-listbox-id', expect.any(Function))
})
