...
export function getListboxSelectElement(
  listboxIdOrElement: string | HTMLElement,
): HTMLSelectElement {
  ...
  if (typeof listboxIdOrElement === "string") {
    ...
    if (!element) {
      throw new ListboxError(
        `Listbox with id "${listboxIdOrElement}" does not exist in the document`,
      );
    }
  ...
  if (!(selectElement instanceof HTMLSelectElement)) {
    const description =
      typeof listboxIdOrElement === "string"
        ? `Listbox with id "${listboxIdOrElement}"`
        : "Listbox element";
    throw new ListboxError(
