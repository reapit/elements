...
function pickStorybookDir({ INPUTS, PKG, SB_CONFIG_DIR }) {
  ...
  try {
    ...
    for (const s of Object.values(scripts)) {
      const m =
        typeof s === "string" &&
        s.match(/\bstorybook\s+(?:dev|build)\b[^;&|]*?(?:-c|--config-dir)[= ]+(\S+)/);
  ...
  return (
    found[0] ?? (existsSync(join(INPUTS, ".storybook")) ? join(INPUTS, ".storybook") : undefined)
  );
}
...
export async function resolveStorybook(ctx) {
  ...
  if (sbStatic) {
    ...
    if (!hasIframe) {
      ...
      console.error(
        `--storybook-static ${sbStatic} has neither index.json nor manifests/components.json`,
      );
      sbStatic = null;
    }
  }
