// forked from design-sync lib/dts.mjs — @reapit/elements has no root "."
// export/types field (subpath exports only: ./core/*, ./icons/*, …), so
// projectFor's entry fallback ('index.d.ts' relative to pkgDir) resolved to
// a nonexistent repo-root file even though findTypesRoot correctly located
// dist/types. Fix: when pj.types/typings/publishConfig.types is absent, fall
// back to '<typesRoot>/index.d.ts' instead of '<pkgDir>/index.d.ts'.
//
...
function projectFor(pkgDir, typesRoot) {
  ...
  const entry =
    pubEntry && existsSync(join(pkgDir, pubEntry))
      ? join(pkgDir, pubEntry)
      : pj.types || pj.typings
        ? join(pkgDir, pj.types || pj.typings)
        : join(findTypesRoot(pkgDir, pj), 'index.d.ts')
  ...
  if (existsSync(reactTypes)) project.addSourceFileAtPath(reactTypes)
    ...
    console.error(
      '\n[DTS_REACT] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n' +
        ...
        '[DTS_REACT] Fix: `yarn add -D @types/react` then rebuild.\n' +
        '[DTS_REACT] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n',
    )
...
export function exportedNames(pkgDir, pkgJson) {
  const typesRoot = findTypesRoot(pkgDir, pkgJson)
  const { project, entry } = projectFor(pkgDir, typesRoot)
  const sf = project.getSourceFile(entry)
  const names = new Set()
  if (sf)
    for (const [name, decls] of sf.getExportedDeclarations()) {
      if (!/^[A-Z][A-Za-z0-9]*$/.test(name)) continue
      if (hasValueDecl(decls)) names.add(name)
    }
  // Supplemental: the top-level aggregate barrel's `export *` list can be
  // incomplete (a component folder present on disk but missing from the
  // parent index.d.ts — an upstream barrel-generation gap, not a
  // design-sync issue). Every already-parsed sub-barrel (category/component
  // /index.d.ts) is scanned directly so those components aren't silently
  // dropped from title-matching.
  for (const subSf of project.getSourceFiles(`${typesRoot}/*/*/index.d.ts`)) {
    for (const [name, decls] of subSf.getExportedDeclarations()) {
      if (/^[A-Z][A-Za-z0-9]*$/.test(name) && hasValueDecl(decls)) names.add(name)
    }
  }
  return names
}
...
export function propsBodyFor(name, ctx) {
  ...
  if (!decl) {
    ...
    const exp =
      decls.find((d) => Node.isVariableDeclaration(d) || Node.isFunctionDeclaration(d) || Node.isClassDeclaration(d)) ??
      decls[0]
...
export function smartDefaultProps(name, pb) {
  ...
  for (const m of body.matchAll(/^ {2}([a-zA-Z_$][\w$]*)(\??)\s*:\s*(.+);$/gm)) {
    ...
    if (prop === 'children' && /React\.ReactNode|ReactElement/.test(t) && !isFn && !VOID_LIKE.test(name))
      props.children = name
    // Visibility toggles — an overlay/dialog with open=false renders nothing.
    else if (/^(open|isOpen|visible|show|defaultOpen|expanded|checked|active|selected)$/.test(prop) && t === 'boolean')
      props[prop] = true
