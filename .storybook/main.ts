...
const layerOrder = fs.readFileSync(
  new URL("../src/styles/layer-order.css", import.meta.url),
  "utf-8",
);
...
export default defineMain({
  ...
  experimental_manifests: (manifests: any) => {
    const components: Record<string, { path: string; import?: string }> | undefined =
      manifests?.components?.components;
