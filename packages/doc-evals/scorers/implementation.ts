...
/**
 * LLM-as-judge scorer for implementation correctness.
 *
 * Evaluates each criterion in the expected list independently.
 * Score = proportion of criteria met (0–1).
 */
export const implementationScorer = createScorer<unknown, string, string[]>({
  name: "Implementation Correctness",
  description:
    "Judges whether the agent produced correct component imports, valid props, proper JSX composition, and sound code patterns.",
  scorer: async ({ output, expected: criteria }) => {
    if (!criteria || criteria.length === 0)
      throw new Error("implementationScorer requires expected criteria");
    ...
    const { object } = await generateObject({
      ...
      prompt: `You are evaluating an AI agent's response to a UI development task. The agent had access to Storybook component documentation only.
