...
test("getIntlNumberFormat does not throw for an out-of-range maximumFractionDigits", () => {
  ...
  expect(getIntlNumberFormat("en-GB", { maximumFractionDigits: 200 })).toBeInstanceOf(
    Intl.NumberFormat,
  );
});
...
test("getIntlNumberFormat does not throw when minimumFractionDigits exceeds maximumFractionDigits", () => {
  expect(() =>
    getIntlNumberFormat("en-GB", { minimumFractionDigits: 5, maximumFractionDigits: 2 }),
  ).not.toThrow();
  expect(
    getIntlNumberFormat("en-GB", { minimumFractionDigits: 5, maximumFractionDigits: 2 }),
  ).toBeInstanceOf(Intl.NumberFormat);
});

// bigint values

test("formats a bigint with group separators", () => {
  expect(getIntlNumberFormat("en-GB").format(BigInt("12345678901234567"))).toBe(
...
test("getNumberAffix returns the £ symbol as a prefix for GBP in en-GB", () => {
  expect(getNumberAffix(0, "en-GB", { style: "currency", currency: "GBP" })).toEqual({
    affix: "£",
    position: "prefix",
  });
});
...
test("getNumberAffix returns the $ symbol as a prefix for USD in en-US", () => {
  expect(getNumberAffix(0, "en-US", { style: "currency", currency: "USD" })).toEqual({
    affix: "$",
    position: "prefix",
  });
});
...
test("getNumberAffix returns the € symbol as a suffix for EUR in de-DE", () => {
  expect(getNumberAffix(0, "de-DE", { style: "currency", currency: "EUR" })).toEqual({
    affix: "€",
    position: "suffix",
  });
});
...
test('getNumberAffix respects an explicit currencyDisplay of "code"', () => {
  const result = getNumberAffix(0, "en-GB", {
    style: "currency",
    currency: "GBP",
...
test('getNumberAffix respects an explicit currencyDisplay of "name"', () => {
  const result = getNumberAffix(0, "en-US", {
    style: "currency",
    currency: "USD",
...
test("getNumberAffix does not throw for an invalid locale", () => {
  expect(() =>
    getNumberAffix(0, "not-a-valid-locale!!", { style: "currency", currency: "GBP" }),
  ).not.toThrow();
});
