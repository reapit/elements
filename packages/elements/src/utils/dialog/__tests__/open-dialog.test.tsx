...
test("throws error when element is not found by ID", () => {
  expect(() => openDialog("non-existent")).toThrow(
    'openDialog: Element with ID "non-existent" not found in the DOM',
  );
});
...
test("throws error when element is not an HTMLDialogElement (by ID)", () => {
  ...
  expect(() => openDialog("test-popup")).toThrow(
    'openDialog: Element with ID "test-popup" is not an HTMLDialogElement',
  );
});
