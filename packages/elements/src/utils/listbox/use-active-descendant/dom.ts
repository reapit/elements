    ...
    case "ArrowDown":
      if (ariaOrientation !== "horizontal") {
        ...
        activateAndMaybeSelect(
          activeOption
            ? getNextOption(options, activeOption)
            : getInitialActiveOption(listboxElement),
        );
      }
      break;
    case "ArrowUp":
      if (ariaOrientation !== "horizontal") {
        ...
        activateAndMaybeSelect(
          activeOption
            ? getPrevOption(options, activeOption)
            : getInitialActiveOptionFromEnd(listboxElement, options),
        );
      }
      break;
    case "ArrowRight":
      if (ariaOrientation === "horizontal") {
        ...
        if (activeOption.matches('summary[role="treeitem"]')) {
          const details =
            activeOption.parentElement instanceof HTMLDetailsElement
              ? activeOption.parentElement
              : null;
          if (details) {
            if (!details.open) {
              details.open = true;
