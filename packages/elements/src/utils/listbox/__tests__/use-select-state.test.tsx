...
test("uses defaultValue when provided as array", () => {
  const { result } = renderHook(() =>
    useListboxSelectState({ defaultValue: ["a", "b"], multiple: true }),
  );
...
test("uses defaultValue when provided as string", () => {
  const { result } = renderHook(() =>
    useListboxSelectState({ defaultValue: "a", multiple: false }),
  );
  const [value] = result.current;
...
test("controlled value always takes precedence", () => {
  const { result } = renderHook(() =>
    useListboxSelectState({ value: ["x"], defaultValue: ["a", "b"], multiple: true }),
  );
  const [value] = result.current;
  expect(value).toEqual(["x"]);
});
...
test("returns only first value when multiple is false", () => {
  const { result } = renderHook(() =>
    useListboxSelectState({ defaultValue: ["a", "b", "c"], multiple: false }),
  );
  const [value] = result.current;
  expect(value).toEqual(["a"]);
});
...
test("returns all values when multiple is true", () => {
  const { result } = renderHook(() =>
    useListboxSelectState({ defaultValue: ["a", "b", "c"], multiple: true }),
  );
  const [value] = result.current;
...
test("updates internal state when change handler is invoked", () => {
  ...
  const { result } = renderHook(() =>
    useListboxSelectState({ defaultValue: ["old"], multiple: true }),
  );
...
test("normalizes string defaultValue to array internally", () => {
  const { result } = renderHook(() =>
    useListboxSelectState({ defaultValue: "single", multiple: false }),
  );
  const [value] = result.current;
