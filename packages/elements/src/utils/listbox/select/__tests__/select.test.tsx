...
test("renders options for selected values to preserve selection state", () => {
  ...
  expect(
    screen.getByRole("option", { name: "Different Option", hidden: true }),
  ).toBeInTheDocument();
});
...
describe("in single-select mode", () => {
  test("renders placeholder option as first option", () => {
    ...
    expect(
      screen.getByRole("option", { name: "Select an item", hidden: true }),
    ).toBeInTheDocument();
  });
  ...
  test("uses default placeholder text when not provided", () => {
    ...
    expect(
      screen.getByRole("option", { name: "Select an option", hidden: true }),
    ).toBeInTheDocument();
  });
...
describe("in multi-select mode", () => {
  test("does not render placeholder option", () => {
    ...
    expect(
      screen.queryByRole("option", { name: "Select items", hidden: true }),
