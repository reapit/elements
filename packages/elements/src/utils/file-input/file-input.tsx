...
export namespace FileInput {
  ...
  export interface Props extends Omit<InputHTMLAttributes<HTMLInputElement>, AttributesToOmit> {
     ...
     *
     * Defaults to `1` when `required` is set and this is otherwise omitted, because `required` is
     ...
     * input's visible content, because a visually-hidden input is still a real, keyboard-operable one,
 ...
 * element, because browsers only let script clear a file input's value, never set it to a chosen file, so
 ...
 * input (`FileUploader` does exactly this) without ending up with two labels, because a native input's
