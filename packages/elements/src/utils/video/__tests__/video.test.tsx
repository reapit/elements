...
test("calls onError when the video fails to load", () => {
  const onError = vi.fn();
  render(
    <Video height="300px" onError={onError} src="https://example.com/video.mp4" width="200px" />,
  );
  const video = document.querySelector("video")!;
  fireEvent.error(video);
...
test("keeps fallback visible until a subsequent loadedData event", () => {
  const { rerender } = render(
    <Video height="300px" src="https://example.com/invalid.mp4" width="200px" />,
  );
  const video = document.querySelector("video")!;
  fireEvent.error(video);
  expect(screen.getByText("The video could not be loaded")).toBeVisible();
...
test("applies data-object-fit attribute to the video element", () => {
  render(
    <Video height="300px" objectFit="cover" src="https://example.com/video.mp4" width="200px" />,
  );
  const video = document.querySelector("video")!;
