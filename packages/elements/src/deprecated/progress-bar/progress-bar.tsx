...
/** @deprecated */
export const ProgressBarPercentage: FC<ProgressBarPercentageProps> = ({
  duration,
  showLabel = true,
  ...rest
}) => {
...
/** @deprecated */
export const ProgressBarSteps: FC<ProgressBarStepProps> = ({
  numberSteps,
  currentStep,
  showLabel = true,
  ...rest
  ...
  const [percentageComplete, setPercentageComplete] = useState<number>(
    (currentStep / numberSteps) * 100,
  );

  useEffect(handleSetPercentageCompleteSteps(setPercentageComplete, currentStep, numberSteps), [
    currentStep,
  ]);
