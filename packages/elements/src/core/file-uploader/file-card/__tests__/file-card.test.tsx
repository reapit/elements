...
test("renders the filename", () => {
  render(
    <FileUploaderFileCard fileName="Invoice.pdf" status="queued" leadingElement={leadingElement} />,
  );
...
test.each(["queued", "uploaded", "error"] as const)(
  "renders no progress bar while %s",
  (status) => {
    render(
      <FileUploaderFileCard
        fileName="Invoice.pdf"
