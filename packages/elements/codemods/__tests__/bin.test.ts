...
vi.mock("../codemods.js", () => ({
  ...
  validateCodemodName: vi.fn(
    (name: string) => name === "my-codemod" || name === "known-but-missing-loader",
  ),
...
describe("handleApply", () => {
  ...
  test("exits with code 1 when loader is missing for a validated codemod name", async () => {
    ...
    await expect(() => handleApply(["known-but-missing-loader", "src"])).rejects.toThrow(
      "process.exit(1)",
    );
    expect(exitCode).toBe(1);
    expect(consoleOutput.join("\n")).toContain(
      "No transform found for codemod 'known-but-missing-loader'",
    );
  });
});
