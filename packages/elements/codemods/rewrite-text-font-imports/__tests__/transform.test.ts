...
test("transforms multiple Text-related imports", () => {
  ...
  const output = transform(input, "test.tsx");
  expect(output).toBe(
    `import { Text, TextColour, textColours } from '@reapit/elements/utils/text'\n`,
  );
});
...
test("transforms multiple font-related imports", () => {
  ...
  const output = transform(input, "test.tsx");
  expect(output).toBe(
    `import { font, FontSize, FontWeight, fontSizes } from '@reapit/elements/utils/font'\n`,
  );
});
...
test("splits complex mixed import with types", () => {
  ...
  expect(output).toContain(
    `import { font, FontSize, type FontStyle } from '@reapit/elements/utils/font'`,
  );
});
...
test("splits mixed import with all font types", () => {
  ...
  expect(output).toContain(`import { Text } from '@reapit/elements/utils/text'`);
  expect(output).toContain(
    `import { font, FontSize, FontWeight, FontStyle } from '@reapit/elements/utils/font'`,
  );
});
