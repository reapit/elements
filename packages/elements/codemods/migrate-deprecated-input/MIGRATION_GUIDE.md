...
No dedicated file input component exists, and `TextInput`'s `type` prop does not include `file`. Use a native `<input type="file">` wrapped manually in `<FormControl>`, and leave a `// TODO:` comment.
...
- [ ] Remove `hasError` and `intent` props from migrated components; preserve an existing `id` prop for label/ARIA stability (see Rule 1)
