 ...
 * property names.
 */
function neededBreakpointHelpers(propertyNames: string[]): {
  needsAtOrAbove: boolean;
  needsBelow: boolean;
} {
 ...
 * const media = useMediaQuery()
 */
function addTodosForNonDestructuredUseMediaQuery(
  sourceFile: SourceFile,
  useMediaQueryAlias: string,
): void {
 ...
 */
function transformMediaStateProvider(
  sourceFile: SourceFile,
  mediaStateProviderAlias: string,
): void {
...
export default function transform(
  ...
  result = result.replace(
    /^(import\s+.*?from\s+'[^']+\/utils\/(?:match-media|breakpoints)');$/gm,
    "$1",
  );
