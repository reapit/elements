...
describe('EmptyStateAction (direct import)', () => {
  test('rewrites the import and JSX tag, adding default props', () => {
    ...
    expect(output).toContain('<AnchorButton href="/items" size="medium" variant="tertiary" useLinkStyle>')
    expect(output).toContain('</AnchorButton>')
    expect(output).not.toContain('EmptyStateAction')
  })

  test('rewrites self-closing JSX tags', () => {
    const input = [`import { EmptyStateAction } from '@reapit/elements'`, `<EmptyStateAction href="/items" />`].join(
      '\n',
    )
    const output = transform(input, 'file.tsx')
    expect(output).toContain('<AnchorButton href="/items" size="medium" variant="tertiary" useLinkStyle />')
  ...
  test('preserves the import alias and adds default props to the aliased tag', () => {
    const input = [`import { EmptyStateAction as Alias } from '@reapit/elements'`, `<Alias href="/items" />`].join('\n')
    const output = transform(input, 'file.tsx')
    expect(output).toContain(`import { AnchorButton as Alias } from '@reapit/elements/core/button'`)
    expect(output).toContain('<Alias href="/items" size="medium" variant="tertiary" useLinkStyle />')
  ...
  test('supports a facade package', () => {
    const input = [`import { EmptyStateAction } from '@company/ui'`, `<EmptyStateAction href="/items" />`].join('\n')
...
describe('EmptyStateActionButton (direct import)', () => {
  test('rewrites the import and JSX tag, adding default props', () => {
    ...
    expect(output).toContain('<Button onClick={handleClick} size="medium" variant="tertiary" useLinkStyle>')
...
describe('facade package support', () => {
  test('adds the AnchorButton import to the facade package for namespaced JSX', () => {
    const input = [`import { EmptyState } from '@company/ui'`, `<EmptyState.Action href="/items" />`].join('\n')
    const output = transform(input, 'file.tsx', { facadePackage: '@company/ui' })
  ...
  test('preserves a facade subpath specifier for namespaced JSX', () => {
    const input = [`import { EmptyState } from '@company/ui/core'`, `<EmptyState.Action href="/items" />`].join('\n')
    const output = transform(input, 'file.tsx', { facadePackage: '@company/ui' })
