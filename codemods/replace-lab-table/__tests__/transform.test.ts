...
describe('import rewrites', () => {
  ...
  test('preserves unrelated imports', () => {
    const input = [`import { TableBody, Input } from '@reapit/elements'`, `<TableBody />`, `<Input />`].join('\n')
...
describe('component rewrites', () => {
  ...
  test('rewrites TableText to Table.PrimaryData', () => {
    const input = [`import { TableText } from '@reapit/elements'`, `<TableText iconLeft={<i/>}>Name</TableText>`].join(
      '\n',
    )
    const output = transform(input, 'file.tsx')
...
describe('exclusions', () => {
  ...
  test('does not rewrite TableRowSelection', () => {
    const input = [`import { TableRowSelection } from '@reapit/elements'`, `<TableRowSelection />`].join('\n')
    expect(transform(input, 'file.tsx')).toBe(input)
  })
})
