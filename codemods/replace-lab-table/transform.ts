...
const ALIGNMENT_TODO = ' TODO: alignment uses dynamic expression. Verify justifySelf mapping (left->start, right->end).'
...
function addTableImport(sourceFile: SourceFile, tableNamespaceLocalName: string, facadePackage?: string): void {
...
function removeTrackedImports(
  ...
  for (const importDecl of sourceFile.getImportDeclarations().slice()) {
    ...
    for (const namedImport of namedImports) {
      ...
      const localNode = (namedImport.getAliasNode() ?? namedImport.getNameNode()).asKindOrThrow(SyntaxKind.Identifier)
...
function getTableNamespaceLocalName(sourceFile: SourceFile, bindings: Binding[], facadePackage?: string): string {
  const targetSpecifier = facadePackage ?? TARGET_SPECIFIER
...
function mapAlignmentInitialiser(initialiserText?: string): { initialiserText?: string; needsTodo: boolean } {
  if (!initialiserText) {
...
function getComponentReplacement(importedName: string, tableNamespaceLocalName: string): string | undefined {
...
function getAttributeInitialiserText(attribute: import('ts-morph').JsxAttribute): string | undefined {
...
export default function transform(
  ...
  const stmtCommentMeta = new Map<number, { insertPos: number; indent: string; messages: Set<TodoMessage> }>()
