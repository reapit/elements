...
function resolveTopBarLocalName(sourceFile: SourceFile, facadePackage?: string): string {
  ...
  const existing = sourceFile.getImportDeclarations().find((decl) => decl.getModuleSpecifierValue() === targetSpecifier)
  if (existing) {
 ...
 */
type IsActiveResult = { kind: 'absent' } | { kind: 'true' } | { kind: 'false' } | { kind: 'dynamic'; text: string }
...
function buildTodoEntry(node: Node, sourceFile: SourceFile, message: string): TodoEntry | undefined {
...
function addTopBarImport(sourceFile: SourceFile, topBarLocalName: string, targetSpecifier: string): void {
  ...
  const alreadyPresent = targetDecl.getNamedImports().some((n) => {
    return n.getName() === 'TopBar' && (n.getAliasNode()?.getText() ?? 'TopBar') === topBarLocalName
  })
...
function transformImports(sourceFile: SourceFile, facadePackage?: string): void {
  ...
  if (!hasExistingTopBarImport) {
    ...
    const targetSpecifier = facadePackage ? (sourceSpecifierWithMobileNavItem ?? facadePackage) : TARGET_SPECIFIER
...
function transformTypeReferences(sourceFile: SourceFile, aliases: Set<string>, topBarLocalName: string): void {
  ...
  for (const exprWithTypes of sourceFile.getDescendantsOfKind(SyntaxKind.ExpressionWithTypeArguments)) {
...
function detectVariant(node: JsxOpeningLike, jsxChildren: import('ts-morph').JsxChild[]): MobileNavItemVariant {
  ...
  const hasOnClick = attrs.some((a) => Node.isJsxAttribute(a) && a.getNameNode().getText() === 'onClick')
  const hasLabel = attrs.some((a) => Node.isJsxAttribute(a) && a.getNameNode().getText() === 'label')
  ...
  for (const child of children) {
    ...
    if (Node.isJsxSelfClosingElement(child)) {
      ...
      if (aliases.has(tagName)) {
        ...
        if (variant === 'anchor') {
          childrenText += buildSubmenuItemReplacement(child, sourceFile, topBarLocalName, todoEntries)
        } else if (variant === 'button') {
          childrenText += buildSubmenuItemButtonReplacement(child, sourceFile, topBarLocalName, todoEntries)
        } else {
...
function transformJsx(sourceFile: SourceFile, aliases: Set<string>, topBarLocalName: string): TodoEntry[] {
