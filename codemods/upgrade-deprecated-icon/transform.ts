 ...
 */
function mapIconName(
  iconName: string,
): { fileName: string; componentName: string; removed?: boolean } | null {
...
function addIconImports(
  ...
  for (const { componentName, fileName } of iconUsages.values()) {
    ...
    if (existingImport) {
      ...
      const hasImport = existingImport
        .getNamedImports()
...
function removeUnusedDeprecatedIconImport(
  ...
  for (const importDecl of importDeclarations) {
    ...
    const namedImports = importDecl.getNamedImports();
    const deprecatedIconImport = namedImports.find(
      (namedImport) => namedImport.getName() === "DeprecatedIcon",
    );

    if (deprecatedIconImport) {
      // Get the imported name (either the alias or 'DeprecatedIcon')
      const importedName =
        deprecatedIconImport.getAliasNode()?.getText() || deprecatedIconImport.getName();
...
function transformJsxElements(
  ...
  for (const element of jsxElements) {
    ...
    const hasDynamicProps = ["intent", "fontSize", "width", "height"].some((propName) =>
      propsToPreserve.has(propName),
    );
    ...
    if (element.getKind() === SyntaxKind.JsxOpeningElement) {
      ...
      if (parent && parent.getKind() === SyntaxKind.JsxElement) {
        ...
        if (closingElement) {
          ...
          if (
            closingTagNameText === "DeprecatedIcon" ||
            closingTagNameText === deprecatedIconAlias
          ) {
...
function collectDeprecatedIconTodoInsertions(
  ...
  const elementsNeedingComments: Array<{
    position: number;
    message: string;
    useJsxComment: boolean;
  }> = [];
