...
test("transforms multiple named imports", () => {
  ...
  expect(output).toContain(
    `import { ThemeProvider, useTheme } from '@reapit/elements/utils/theme-provider'`,
  );
  expect(output).not.toContain("/core/theme-provider");
});
...
test("transforms type-only import declaration", () => {
  ...
  const output = transform(input, "test.tsx");
  expect(output).toContain(
    `import type { ThemeProviderProps } from '@reapit/elements/utils/theme-provider'`,
  );
  expect(output).not.toContain("/core/theme-provider");
});
...
test("preserves import alias", () => {
  ...
  const output = transform(input, "test.tsx");
  expect(output).toContain(
    `import { ThemeProvider as Provider } from '@reapit/elements/utils/theme-provider'`,
  );
  expect(output).not.toContain("/core/theme-provider");
});
...
test("transforms facade package imports", () => {
  ...
  expect(output).toContain(
    `import { ThemeProvider } from '@company/ui-components/utils/theme-provider'`,
  );
  expect(output).not.toContain("/core/theme-provider");
});
