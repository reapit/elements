...
describe("inline mapping — font family", () => {
  test("inlines --font-sans-serif with full font stack", () => {
    ...
    expect(transform(input)).toBe(
      `font-family: 'Inter', Helvetica, Arial, sans-serif /* was --font-sans-serif */;`,
    );
  });
});
...
describe("inline mapping — font sizes", () => {
  test("inlines --font-size-heading with 1.5rem", () => {
    expect(transform("font-size: var(--font-size-heading);")).toBe(
  ...
  test("inlines --font-size-small with 0.875rem", () => {
    expect(transform("font-size: var(--font-size-small);")).toBe(
...
describe("inline mapping — font weights", () => {
  ...
  test("inlines --font-weight-bold with 600", () => {
    expect(transform("font-weight: var(--font-weight-bold);")).toBe(
...
describe("inline mapping — component input tokens", () => {
  ...
  test("inlines --component-input-shadow", () => {
    ...
    expect(transform(input)).toBe(
      `box-shadow: inset 0 -1px 0 #ffffff /* was --component-input-shadow */;`,
    );
  });
});
...
describe("inline mapping — utility tokens", () => {
  ...
  test("inlines --util-box-shadow", () => {
    ...
    expect(transform(input)).toBe(
      `box-shadow: 0 2px 9px rgb(0 0 0 / 0.08) /* was --util-box-shadow */;`,
    );
  });
...
describe("inline mapping — other tokens", () => {
  ...
  test("inlines --font-monospace", () => {
    ...
    expect(transform(input)).toBe(
      `font-family: 'Source Code Pro', monospace /* was --font-monospace */;`,
    );
  });
...
describe("multiple replacements", () => {
  ...
  test("handles a mix of mapped, inline, and unmapped variables", () => {
    const input =
      "color: var(--intent-primary); background: var(--component-input-bg); z-index: var(--z-index-sticky);";
    const output = transform(input);
    expect(output).toContain("var(--colour-fill-action-dark)");
...
describe("legacy-reapit/tokens.css mappings", () => {
  ...
  test("replaces --fill-white with --colour-fill-white", () => {
    expect(transform("background: var(--fill-white);")).toBe(
  ...
  test("replaces --icon-checkbox-hover with consolidated --colour-icon-hover", () => {
    expect(transform("color: var(--icon-checkbox-hover);")).toBe(
      "color: var(--colour-icon-hover);",
    );
  });
  ...
  test("replaces --corner-none with --border-radius-none", () => {
    expect(transform("border-radius: var(--corner-none);")).toBe(
      "border-radius: var(--border-radius-none);",
    );
  });

  test("replaces --corner-lg with --border-radius-l", () => {
    expect(transform("border-radius: var(--corner-lg);")).toBe(
  ...
  test("replaces --font-size-base with --font-base-regular-size", () => {
    expect(transform("font-size: var(--font-size-base);")).toBe(
      "font-size: var(--font-base-regular-size);",
    );
  });

  test("replaces --line-height-2xs with --font-2xs-regular-line_height", () => {
    expect(transform("line-height: var(--line-height-2xs);")).toBe(
      "line-height: var(--font-2xs-regular-line_height);",
    );
  });
