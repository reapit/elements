...
describe("namespaced component usage (AtAGlance.Card)", () => {
  ...
  test("preserves all props when renaming AtAGlance.Card to AtAGlance.ArticleCard", () => {
    const input =
      '<AtAGlance.Card displayValue="42" label="Total" description="Last 30 days" layout="horizontal" />';
    const output = transform(input);
    expect(output).toBe(
  ...
  test("does not rename AtAGlance.Card when it has children", () => {
    const input =
      "<AtAGlance.Card><AtAGlance.CardLabel>Label</AtAGlance.CardLabel></AtAGlance.Card>";
    const output = transform(input);
    expect(output).toBe(input);
  });
  ...
  test("does not modify non-AtAGlance Card components", () => {
    const input =
      '<><AtAGlance.Card displayValue="42" label="Total" /><Card title="Other">Content</Card></>';
    const output = transform(input);
