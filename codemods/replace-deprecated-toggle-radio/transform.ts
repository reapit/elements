 ...
 */
function getAttr(element: JsxOpeningElement | JsxSelfClosingElement, name: string): JsxAttribute | undefined {
...
function expandOptionsToChildren(
  ...
  const baseIndent = lastNewline === -1 ? '' : (textBefore.slice(lastNewline + 1).match(/^(\s*)/)?.[1] ?? '')
  ...
  for (const attr of element.getAttributes()) {
    ...
    if (attrName === 'options' || attrName === 'disabled' || attrName === 'hasGreyBg' || attrName === 'isFullWidth') {
      continue
    }
...
function transformImports(sourceFile: SourceFile, facadePackage?: string): Set<string> {
  const aliases = getImportAliases(sourceFile, 'ToggleRadio', facadePackage, { fallbackToName: true })
...
function replaceWithDynamicOptionsPlaceholder(
  ...
  for (const attr of element.getAttributes()) {
    ...
    const attrName = attrNode.getNameNode().getText()
    if (attrName === 'options' || attrName === 'hasGreyBg' || attrName === 'isFullWidth' || attrName === 'disabled') {
      continue
    }
...
function transformJsxElements(
  ...
  const elements: Array<JsxOpeningElement | JsxSelfClosingElement> = [...allOpeningElements, ...allSelfClosing].filter(
    (el) => aliases.has(el.getTagNameNode().getText()),
  )
  ...
  for (const element of elements) {
    ...
    const hasRemovedProps = getAttr(element, 'hasGreyBg') !== undefined || getAttr(element, 'isFullWidth') !== undefined
