...
import { getNearestStatement, collectStatementCommentPositions, insertLineComments } from '../comments.js'
import { renameProp, removeProp, addProp, getPropStringValue, applyPropTransforms } from '../props.js'
...
describe('resolveTargetSpecifier', () => {
  ...
  test('returns the sourceSpecifier when it matches the facadePackage exactly', () => {
    const result = resolveTargetSpecifier('@company/ui', '@reapit/elements/core/button', '@company/ui')
    expect(result).toBe('@company/ui')
  })

  test('returns targetSpecifier when facadePackage does not match', () => {
    const result = resolveTargetSpecifier('@reapit/elements', '@reapit/elements/core/button', '@company/ui')
    expect(result).toBe('@reapit/elements/core/button')
  })

  test('returns sourceSpecifier for a subpath facade import', () => {
    const result = resolveTargetSpecifier('@company/ui/core', '@reapit/elements/core/button', '@company/ui')
...
describe('addImportsToTarget', () => {
  ...
  test('adds an aliased import', () => {
    const sf = createSourceFile(`const x = 1`)
    addImportsToTarget(sf, [{ name: 'LabelText', alias: 'L', isTypeOnly: false }], '@reapit/elements')
...
describe('hasIdentifierUsage', () => {
  ...
  test('returns true for a bare export with treatLocalReExportsAsUsage: true', () => {
    const sf = createSourceFile(`export { Button }`)
    expect(hasIdentifierUsage(sf, new Set(['Button']), { treatLocalReExportsAsUsage: true })).toBe(true)
  })

  test('returns false for a re-export with module specifier even when treatLocalReExportsAsUsage: true', () => {
    const sf = createSourceFile(`export { Button } from '@reapit/elements'`)
    expect(hasIdentifierUsage(sf, new Set(['Button']), { treatLocalReExportsAsUsage: true })).toBe(false)
  })
})
