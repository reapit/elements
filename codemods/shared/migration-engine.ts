...
export interface ComponentMigrationConfig<TCtx = Record<string, unknown>> {
   ...
   */
  afterTransform?: (sourceFile: SourceFile, context: { facadePackage?: string; context: TCtx }) => void
}
...
export function createComponentMigration<TCtx = Record<string, unknown>>(
  ...
  return function transform(
    ...
    for (const identifier of identifiers) {
      const useFallback = fallbackIdentifiers === undefined || fallbackIdentifiers.has(identifier.from)
