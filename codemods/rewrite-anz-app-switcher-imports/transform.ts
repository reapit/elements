...
function isAppSwitcherImport(specifier: string, facadePackage?: string): boolean {
  ...
  if (facadePackage !== undefined && specifier === `${facadePackage}/core/app-switcher`) return true
  return false
}
...
function hasAnzPropertyAccess(sourceFile: ReturnType<typeof createProjectFromSource>, localName: string): boolean {
  return sourceFile.getDescendantsOfKind(SyntaxKind.PropertyAccessExpression).some((node) => {
    return node.getExpression().getText() === localName && ANZ_MEMBER_PROPERTIES.has(node.getName())
  })
}
...
function transform(source: string, filePath: string = 'file.tsx', options?: { facadePackage?: string }): string {
  ...
  for (const importDecl of sourceFile.getImportDeclarations()) {
    ...
    const hasAnzMemberAccess = localName !== undefined && hasAnzPropertyAccess(sourceFile, localName)
