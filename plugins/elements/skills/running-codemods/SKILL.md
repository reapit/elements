...
Reach for it when the changelog references a migration path, or you're hitting breaking changes, deprecation warnings, or TypeScript errors after an upgrade. Skip it for new features or bug fixes — there's nothing to migrate.
...
- **Forgetting `--facade-package`** when Elements is re-exported through an internal wrapper — otherwise the codemod finds nothing to transform.
