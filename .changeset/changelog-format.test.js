...
test('infers "Added" category for a minor bump with no prefix', async () => {
  ...
  const result = await getReleaseLine(
    makeChangeset({ summary: "New button variant" }),
    "minor",
    OPTIONS,
  );
...
test('infers "Fixed" category for a patch bump with no prefix', async () => {
  ...
  const result = await getReleaseLine(
    makeChangeset({ summary: "Correct icon alignment" }),
    "patch",
    OPTIONS,
  );
...
test('infers "Removed" category for a major bump with no prefix', async () => {
  ...
  const result = await getReleaseLine(
    makeChangeset({ summary: "Remove DeprecatedButton" }),
    "major",
    OPTIONS,
  );
...
test("prefix matching is case-insensitive", async () => {
  ...
  const result = await getReleaseLine(
    makeChangeset({ summary: "ADDED: Dark mode support" }),
    "patch",
    OPTIONS,
  );
...
test("conventional commit prefix is treated as part of the description", async () => {
  ...
  const result = await getReleaseLine(
    makeChangeset({ summary: "feat: Add dark mode support" }),
    "patch",
    OPTIONS,
  );
...
test("appends only the PR link to the entry", async () => {
  vi.mocked(getCommitInfo).mockResolvedValue(
    makeInfo({
      pull: 1105,
      commit: "e78c4c8234416385c2546464b2c5399bd6ace088",
      user: "kdoherty_Reapit",
    }),
  );
...
test("handles a single-line summary without trailing newline", async () => {
  ...
  const result = await getReleaseLine(
    makeChangeset({ summary: "fix: Correct focus ring colour" }),
    "patch",
    OPTIONS,
  );
...
test("throws when repo option is missing", async () => {
  await expect(getReleaseLine(makeChangeset(), "patch", null)).rejects.toThrow(
    "Please provide a repo",
  );
});

test("throws when options object is provided but repo is absent", async () => {
  await expect(getReleaseLine(makeChangeset(), "patch", {})).rejects.toThrow(
    "Please provide a repo",
  );
});
...
test("throws when repo option is missing from getDependencyReleaseLine", async () => {
  await expect(
    getDependencyReleaseLine([makeChangeset()], [makeDep("pkg", "1.0.0")], null),
