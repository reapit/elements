...
function findBaselineYear(browser: string, version: string, floors: Map<string, Map<number, string>>): number | null {
...
async function fetchEndOfLife(product: string): Promise<EndOfLifeCycle[]> {
  ...
  if (!res.ok) throw new Error(`Failed to fetch ${product} from endoflife.date: HTTP ${res.status}`)
